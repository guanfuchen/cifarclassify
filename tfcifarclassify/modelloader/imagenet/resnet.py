# -*- coding: utf-8 -*-
# code is from [resnet_model.py](https://github.com/tensorflow/models/blob/master/research/resnet/resnet_model.py)
...
# Hyper Parameters
HParams = namedtuple('HParams', 'batch_size, num_classes, min_lrn_rate, lrn_rate, num_residual_units, use_bottleneck, weight_decay_rate, relu_leakiness, optimizer')
...
class ResNet(object):
  ...
  def _build_model(self):
    ...
    with tf.variable_scope('init'):
      x = self._images
      x = self._conv('init_conv', x, filter_size=3, in_filters=3, out_filters=16, strides=self._stride_arr(1))

    strides = [1, 2, 2]
    activate_before_residual = [True, False, False] # relu(F+x) or x+relu(F)

    # check whether use bottleneck
  ...
  def _residual(self, x, in_filter, out_filter, stride, activate_before_residual=False):
    ...
    with tf.variable_scope('sub_add'):
      # when in_filter is not equal to out_filter, use average pool
  ...
  def _relu(self, x, leakiness=0.0):
    ...
    """
    # if x<0 then leakiness * x otherwise x
