...
class wide_resnet(nn.Module):
    def __init__(self, depth, width, n_classes):
        ...
        self.n = (depth - 4) // 6
        widths = [int(v * width) for v in (64, 128, 256, 512)]  # for normal resnet 16 32 64
        ...
        self.flat_params = cast(flatten({
            ...
            'group3': gen_group_params(widths[2], widths[3], self.n),
            'bn': bnparams(widths[3]),
            'fc': linear_params(widths[3], n_classes),
        }))
    ...
    def forward(self, input):
        x = F.conv2d(input, self.flat_params['conv0'], padding=1, stride=2)
        # print('x.shape:', x.shape)
        g0 = self.group(x, self.flat_params, 'group0', self.training, 2)
        ...
        g3 = self.group(g2, self.flat_params, 'group3', self.training, 2)
        o = F.relu(batch_norm(g3, self.flat_params, 'bn', self.training))
...
def wide_resnet_16_4(n_classes):
    depth = 16
    width = 4
    n_classes = n_classes
    return wide_resnet(depth, width, n_classes)
...
if __name__ == '__main__':
    ...
    n_classes = 1000
    # model = wide_resnet_28_10(n_classes)
    model = wide_resnet_16_4(n_classes)
    # model.eval()
    ...
    print("Wide ResNet forward time:", end-start)
